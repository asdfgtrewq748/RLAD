...
class CoreMetricsVisualizer:
    ...
    def plot_attention_weights(self, agent, sample_data, device, save_path=None):
        ...
        for head_idx in range(num_heads):
            ax = fig.add_subplot(gs[head_idx, 0])
...
class EnhancedRLADAgent(nn.Module):
    ...
    def forward(self, x, return_features=False, return_attention_weights=False):
        """优化的前向传播 - 提高数值稳定性，内部处理增强特征"""
        
        # 🔥 修复：确保输入是3D张量 [batch_size, seq_len, features]
        if len(x.shape) == 2:  # [batch_size, features]
            # 这种情况不应该出现，但我们做个保护
            batch_size, total_features = x.shape
            # 重塑为时序格式
            seq_len = min(288, total_features)
            x = x[:, :seq_len].unsqueeze(-1)  # [batch_size, seq_len, 1]
...
def enhanced_compute_reward(action, label, is_human_labeled=False, is_augmented=False):
    """
    🔥 增强版奖励计算 - 强化"安全第一"原则，大幅加大漏报惩罚
    根据您的建议，将FN惩罚从-3.0调整到-5.0，进一步减少漏报
    """
    # 确保动作和标签格式正确
    action = int(action)
    label = int(label)
    ...
    if label == 1:  # 异常样本
        if action == label:  # 正确预测异常 (TP)
            base_reward = 2.0  # 提高对异常样本正确检测的奖励
        else:  # 错误地预测为正常 (FN - 漏报)
            base_reward = -5.0  # 🚨 大幅增加漏报惩罚 (从-2.0增加到-5.0)
    else:  # 正常样本
        if action == label:  # 正确预测正常 (TN)
            base_reward = 1.0  # 保持对正常样本的标准奖励
        else:  # 错误地预测为异常 (FP - 误报)
            base_reward = -1.0  # 适度降低误报惩罚，鼓励模型更倾向于异常检测
    
    # 🔥 新增：基于混淆矩阵平衡的动态奖励调整
    # 如果当前批次中漏报较多，进一步加大漏报惩罚
    ...
    # 🔥 新增：连续性奖励 - 鼓励一致的异常检测
    # 这部分需要在调用时传入上下文信息，暂时保留接口
    
    # 人工标注的样本额外奖励
    if is_human_labeled:
        base_reward *= 1.5  # 提高人工标注样本的权重
        
    # 增强的样本略微降低奖励
    if is_augmented:
        base_reward *= 0.85  # 稍微降低增强样本的权重
...
def main():
    ...
    # 优化参数 - 稳定性改进的超参数
    parser.add_argument('--lr', type=float, default=3e-4, help='稳定的学习率')  # 提高学习率
    parser.add_argument('--batch_size_rl', type=int, default=32, help='增大批次大小提高稳定性')  # 增大批次
    parser.add_argument('--target_update_freq', type=int, default=10, help='减少目标网络更新频率')  # 减少更新频率
    parser.add_argument('--epsilon_start', type=float, default=0.5, help='适中的初始探索率')
    parser.add_argument('--epsilon_end', type=float, default=0.01, help='更低的最终探索率')
    parser.add_argument('--epsilon_decay_rate', type=float, default=0.995, help='更平缓的epsilon衰减')
    parser.add_argument('--gamma', type=float, default=0.99, help='提高折扣因子')  # 提高gamma
    
    # 新增训练稳定性参数
    parser.add_argument('--grad_clip', type=float, default=0.5, help='降低梯度裁剪阈值')
    parser.add_argument('--loss_clip', type=float, default=0.5, help='降低损失裁剪阈值')  # 降低loss clip
    parser.add_argument('--weight_decay', type=float, default=1e-4, help='适中的权重衰减')  # 减少weight decay
    parser.add_argument('--early_stopping', type=int, default=20, help='增加早停耐心')  # 增加耐心
    parser.add_argument('--scheduler_patience', type=int, default=8, help='增加LR调度耐心')  # 增加耐心
    parser.add_argument('--scheduler_factor', type=float, default=0.8, help='更温和的LR衰减')  # 更温和
    parser.add_argument('--dropout', type=float, default=0.3, help='增加dropout防止过拟合')  # 增加dropout
    ...
    try:
        ...
        # 保留原有的ReduceLROnPlateau作为备选调度器
        plateau_scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode='max',
            factor=args.scheduler_factor,
            patience=args.scheduler_patience,
            threshold=0.01,  # 增大阈值，减少过度调整
            min_lr=1e-5,     # 提高最小学习率
            verbose=True
        )
